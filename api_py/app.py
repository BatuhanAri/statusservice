...
def index():
    idx = WWW / "index.html"
    return FileResponse(str(idx)) if idx.exists() else JSONResponse({"hint":"put www/index.html"})
...
async def check_one(t:Dict[str,Any], timeout_ms:int)->Dict[str,Any]:
    ...
    # Rozetler için metadata (UI gösterimi)
    ...
    def present_auto() -> bool:
        # HTTP hedeflerde HTTP’e, diğerlerinde TCP’e bak
        if has_http:
    ...
    def present_http() -> bool:
        # HTTP kontrolü tanımlı değilse auto’ya düş
        if not has_http:
            return present_auto()
        return bool(res.get("http_ok") is True)
